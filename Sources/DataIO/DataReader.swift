...
public struct DataReader {
    ...
    public mutating func set<SomeType: BinaryInteger>(_ newOffset: SomeType) {
        offset = Int(newOffset)
    }
    
    public mutating func skip<SomeType: BinaryInteger>(_ bytesToSkip: SomeType) {
        let numBytes = Int(bytesToSkip)
        offset += numBytes
    }
    
    public mutating func back(_ bytesToGoBack: Int) {
        offset -= bytesToGoBack
    }
    
    public mutating func read<SomeType>() throws -> SomeType {
    ...
    public mutating func readData<Integer: BinaryInteger>(ofLength length: Integer) throws -> Data {
        guard length >= 0 else {
